...
// 归一化只写一份：createLlm 与界面端点都从这里取，避免两处解释两处漂。
...
export const DEFAULTS = {
  ...
  // score >= 此值 → weak（只注入标题索引）；低于 → miss。
  //
  // ★ 0.13 → **0.15**（2026-09-16 重标定，66 页语料 + 13 正例 / 5 负例）。
  //   不是"为了让测试变绿"，而是实测扫出来的：0.13 时仍有一条负例进 weak
  //   （注入低置信噪声），提到 0.15 后 **负例零注入、正例 13/13 全注入**，
  //   而 0.18 就会开始漏掉正例。可复现：scripts/calibrate-thresholds.mjs 逐条打印。
  //
  //   重标定的直接动因是两条负例越线，两条根因都不是阈值：
  //     · 一条**检索到了自己**（页面引用了标定查询）→ 修法是 meta 页不参与自动注入；
  //     · 一条问的技术名词（kubernetes）**整个语料里都不存在** → 修法是缺词折扣。
  //   详见 lib/recall.js 里 recallable 与 absentRatio 两处注释。
  //
  //   ★ 重标定后正例最低 0.1621 / 负例最高 0.1493，间隔 **+0.013** —— 很薄。
  //     语料再显著增长就得重跑 calibrate-real.mjs（看整体）与
  //     calibrate-thresholds.mjs（逐条看），verify-core 里的断言会先红。
  //
  //   ★★ 0.15 → **0.16**（2026-09-17，给 note-ca2525 打上 meta 标签之后语料又动了一次）。
  //     哨兵如期先红：那条负例 0.1493 → **0.1553**，越过 0.15，于是"关于 kubernetes
  //     的查询"会给一个只共享 sidecar/注入/策略 的页面注入标题索引。
  //
  //     这次**量了两种错误的代价**再决定（不是往下调阈值让它变绿）：
  //       weak=0.15 → 正例注入 13/13，负例注入 1/5
  //       weak=0.16 → 正例注入 13/13，负例注入 0/5     ← 零代价地把负例挡掉
  //       weak=0.17 → 正例注入 12/13（开始漏掉一条真实正例）
  //     0.16 是唯一同时满足"负例零注入"与"正例不漏"的点。
  //     把 0.17 那一行也记下来，是为了让下一个人知道**再往上调就要开始丢东西**。
  weakThreshold: 0.16,
  ...
  // 每条正文截断长度。**4000 是实测定出来的错值**，提到 12000：
  //   实测 nodejs.org/api/zlib.html 一篇 zlib 文档能剥出 9 万字符正文，
  //   MDN / GitHub 这类页面"正文只占几个百分点"，4000 字符的预算**先被导航吃掉**，
  //   截断截在噪声上，答案根本没进提示词。12000 字符 ≈ 3k token，
  //   而蒸馏这一步本来就要带 14000 字符的页面内容（见 distillPrompt）。
  fetchMaxChars: 12000,
  fetchTimeoutMs: 15000,    // 单页抓取超时
  // 一页"够不够用"的下限。低于它不算证据（实测 874 字符的 GitHub 壳页就是这种）。
  fetchMinUsableChars: 200,
  // 抽取文本的 prose 比例下限，0 = 不设限（默认保守：让蒸馏器看到再拒绝）。
  // 目录型页面字符数大而 purity 极低，打开这个能挡掉它们，代价是可能误杀。
  fetchMinProse: 0,
  // 抓取缓存：同一批 URL 会被反复抓（gap 两次尝试、相似缺口），实测单页 1.4 秒。
  // 0 = 关闭（测试与离线场景）。
  fetchCacheTtlMs: 86400000,     // 成功 24h
  fetchCacheFailTtlMs: 600000,   // 失败 10 分钟（死链别反复付超时，但也别钉死一天）
  fetchCacheMaxChars: 40000,     // 单条缓存上限（只存抽取后的文本，不存原始 HTML）
  // 总闸：true 时整条补料路径不发任何网络请求。跑测试、演示、省额度时用。
  offline: false,
  // 检索前是否清洗查询词（剥掉"常见故障原因 解决办法"这类**给人读的**指令尾缀）。
  //
  // ★ 这个键以前**不存在**，而 acquire.js 已经在读它：cfg.normalizeSearchQuery === false
  //   恒为 false，清洗照常生效，看起来"没问题"；但那意味着这个开关**只能靠改代码**。
  //   一个只在代码里存在的开关等于没有开关，所以补上声明。
  normalizeSearchQuery: true,
  ...
  // ── 模型：所有调用 LLM 的环节都从这里取路由 ──
  //
  // 两个调用点：distill（联网补料的蒸馏）、harvest（会话提炼）。
  // 它们对模型的要求不一样 —— 蒸馏要能扛 4000 字符的网页正文并吐严格 JSON，
  // 提炼要能在一段对话里认出"哪一句是设计规则"。所以允许分别配。
  //
  //   mode: 'single' 只用 models[0]；'rotate' 每次调用换下一个。
  //   models: 有序候选。空数组 = 跟随宿主默认（不指定任何模型）。
  //           写法："provider/model"、"provider"、或 { provider, model }。
  //   onError: 'next' 失败了问下一个（默认）；'fail' 只问一个，失败就认。
  //   sites:  按站点覆盖，只写要覆盖的字段。
  //           例：{ "harvest": { "models": ["deepseek/deepseek-chat"] } }
  //
  // ★ 以前这里是扁平的 llmProvider/llmModel 两个键，而且 createLlm 读的是
  //   apply() 的参数**不是这个文件** —— 写进 wiki.config.json 毫无反应。
  //   旧键仍然认（见 normalizeLlmConfig），但新配置请用 llm 块。
  llm: {
    mode: 'single',
    models: [],
    onError: 'next',
  ...
  // 旧键：只为不让已有的 wiki.config.json 静默失效，保留读取。
  llmProvider: '',
  llmModel: '',
}
...
export async function loadConfig(wikiRoot, override = {}) {
  ...
  // llm 块必须被收敛成可执行形状才交出去。理由与上面那条同源：
  // 一个 mode 拼错、models 写成对象而不是数组，都不该让**下一次模型调用**
  // 才发现自己在用一份读不懂的配置。归一化放在这里，createLlm 与界面
  // 才能共用同一个解释（两处各解一遍就是两处会漂）。
  cfg.llm = normalizeLlmConfig(cfg.llm, { provider: cfg.llmProvider, model: cfg.llmModel })
  return cfg
}
