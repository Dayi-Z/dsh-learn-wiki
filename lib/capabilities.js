...
export function createCapabilityManager({ ctx, getCfg, log = () => {} }) {
  ...
  const applyTo = (agent) => {
    try {
      ...
      if (missing.length === 0 && !fullCatalog.has(agent)) {
        ...
        if (snap.length > lastSnapshot.length) lastSnapshot = snap
      }
  ...
  /** 给 UI 用：完整目录 + 当前裁掉了哪些（各带 token 估算）。 */
  const catalogSnapshot = (deniedNames = []) => {
    const denied = new Set(deniedNames)
    const items = lastSnapshot.map(s => ({
      name: s.name,
      denied: denied.has(s.name),
      // 粗估：按 M0 标定的经验值 ~50 token/工具，仅供界面排序参考
      approxTokens: Math.max(8, Math.round(String(s.description ?? '').length / 4.5)),
    }))
    return { items, total: items.length, capturedAt: lastSnapshot.length > 0 }
  }
