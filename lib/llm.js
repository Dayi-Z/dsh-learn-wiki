    ...
    async chat({ site = 'distill', system, prompt, maxTokens = 2048, temperature = 0.2 } = {}) {
      ...
      for (let i = 0; i < attempts; i++) {
        ...
        try {
          ...
          let text = ''
          // 诊断用计数。宿主的流有**独立的 reasoning 通道**（chunk 类型里有
          // 'reasoning-delta'，见 dsh-llm 的 assembler.js / assistant-stream.js）。
          // 这里刻意**不把推理内容当正文**（那是草稿不是答案），但要数出来 ——
          // 否则「模型烧光预算只推理不回答」会表现成一个**空成功**，无从诊断。
          ...
          for await (const chunk of stream) {
            ...
            if (chunk && typeof chunk.type === 'string') seenTypes.add(chunk.type)
            if (chunk && chunk.type === 'text-delta' && typeof chunk.text === 'string') text += chunk.text
            else if (chunk && chunk.type === 'reasoning-delta' && typeof chunk.text === 'string') reasoningChars += chunk.text.length
          }
          // ★ 空回复 = **失败**，不是成功。
          //
          //   实测（2026-09-17 重启后）：harvest 拿到 len=0 的输出，报「模型没有返回
          //   合法 JSON」，而真相是这个候选模型把预算全花在 reasoning 上、正文一个字
          //   都没给。旧代码把它当**成功**返回，于是：轮换白配（不会去试下一个候选），
          //   调用方还以为是格式问题。
          //
          //   丢进 catch 走「这次候选失败」的既有路径：onError=next 会去试下一个模型。
          if (!text.trim()) {
            throw new Error('空回复（没收到任何 text-delta'
              + (reasoningChars ? '；只收到 ' + reasoningChars + ' 字符 reasoning' : '')
              + '；chunk 类型=' + ([...seenTypes].join('/') || '(无)') + '）')
          }
...
/**
 * 从模型输出里抠出 JSON 对象——容忍 ```json 围栏、前后废话、以及
 * **思考过程把 JSON 污染了**的情况。
 *
 * ── 这个函数修的是 harvest 的第二次静默失败（2026-09-17）──
 *
 * 实际输出（opencodefree 的推理模型，len=10215）：
 *
 *   Here's a thinking process:
 *   1. **Analyze User Request:** …（一大段思考，里面常带 {…} 示例）…
 *   {实际答案的 JSON}
 *
 * 旧实现两个 bug：
 *   1. 围栏正则 `s*([sS]*?)` 是坏的——`s*` 匹配字面 s、`[sS]` 只匹配 s/S，
 *      等于 ```json 围栏块永远抠不出来（除非内容全是 s）。应该是 \s* 和 [\s\S]。
 *   2. 全宽切片（第一个 { 到最后一个 }）在思考过程先出现 { 时，会把中间的
 *      散文一起切进去，JSON.parse 必挂。
 *
 * 修法：扫描每个 { 的位置，用**配平计数**（感知字符串转义）找出从它开始的最短
 * 配平对象，逐个试 parse；在所有可解析候选中选「闭合位置最靠后、开口最靠前」
 * 的那个 —— 即**最外层且最接近末尾**的对象。思考里的示例对象都在答案前面，
 * 答案（最外层对象）一定闭合在最后，所以这个规则稳定选中答案。
 *
 * ★ 还有一个陷阱（真实样例踩到）：思考过程里可能带一个**完整可解析**的示例
 *   对象（如 {"skip": true}），而真正的答案恰好被 maxTokens **截断**、没有闭合。
 *   此时"闭合最靠后"会选中那个示例 —— 把示例当成结果。这个函数是通用的，
 *   不知道调用方的契约，所以**不做**形状判断；但接受一个可选的 ok 回调，
 *   调用方用契约过滤候选（如 harvest 要求 skip 为布尔且带 items/reason）。
 *   不传 ok = 纯语法提取（distill 这类只要形状差不多的场景）。
 *
 * @param {string} text   模型原始输出
 * @param {(v: any) => boolean} [ok]  契约校验：候选对象必须通过它才被接受。
 */
export function extractJson(text, ok) {
  const s = String(text ?? '')
  // 围栏优先（这次正则是对的：\s* 吃空白，[\s\S] 吃一切）。
  const fenced = s.match(/```(?:json)?\s*([\s\S]*?)```/)
  const good = (v) => v && typeof v === 'object' && (!ok || ok(v))
  const tryParse = (c) => {
    try { const v = JSON.parse(c); if (good(v)) return v } catch { /* 下一个 */ }
    return null
  }
  if (fenced) { const v = tryParse(fenced[1]); if (v) return v }
  const start = s.indexOf('{')
  const end = s.lastIndexOf('}')
  if (start !== -1 && end > start) { const v = tryParse(s.slice(start, end + 1)); if (v) return v }
  // 兜底：配平扫描。最多看 500 个 {（病理输出也不至于更多），答案在末尾，
  // 所以优先记住「闭合最靠后」的候选，闭合一样时选开口最靠前（最外层）。
  let best = null, bestClose = -1, bestOpen = 0, seen = 0
  for (let i = 0; i < s.length && seen < 500; i++) {
    if (s[i] !== '{') continue
    seen++
    ...
    if (close === -1) continue
    const v = tryParse(s.slice(i, close + 1))
    if (v) {
      // 闭合靠后优先；闭合相同选开口靠前（外层）。
      if (close > bestClose || (close === bestClose && i < bestOpen)) { best = v; bestClose = close; bestOpen = i }
    }
  }
  return best
}
