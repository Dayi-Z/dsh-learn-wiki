...
/**
 * 极简 semver 范围：只支持空格分隔的合取（`>=0.1.5-rc.1 <0.2.0-0`）。
 * 不支持 `||` / `^` / `~` —— 遇到了返回 null（"判不了"），
 * 而不是**猜一个答案**。判不了比判错好。
 */
...
/**
 * 按路径取值，**读不到就返回 undefined —— 绝不抛**。
 *
 * ── 为什么必须 try 住（实测，2026-09-17 重启之后在真宿主里炸了）──
 *
 * Cordis 的上下文对**未声明的服务**是抛异常的，不是返回 undefined：
 *
 *     ctx.sessionQuery   ->  Error: cannot get property "sessionQuery" without inject
 *
 * 于是这一行把整个兼容性自检打没了，日志里留下的是：
 *
 *     [dsh-learn-wiki] compat check failed (non-fatal): cannot get property "sessionQuery" without inject
 *
 * 也就是说：**一个诊断把自己要诊断的东西弄没了**。这份报告的全部价值就是
 * "宿主 API 还在不在"，而它恰恰在"某个服务不存在"时最该出现 ——
 * 那正是它被写出来的原因。
 *
 * 先前没抓到是因为单测传的是普通对象（读不存在的键给 undefined）；
 * 真实上下文的语义不一样。所以除了这个 try，还补了一条**会抛的代理**夹具
 * （scripts/verify-compat.mjs），把这条差异钉住。
 *
 * 语义上也对：读不到 == "这项不在"。一个抛异常的宿主不该比一个返回 undefined
 * 的宿主被判得更严重 —— 对插件来说两者都只是"用不了这个服务"。
 */
function dig(obj, path) {
  let cur = obj
  for (const k of String(path).split('.')) {
    if (cur === null || cur === undefined) return undefined
    try { cur = cur[k] } catch { return undefined }
  }
  return cur
}
