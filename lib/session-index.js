...
export async function loadSessionIndex(wikiRoot, {
  ...
  for (const { s, stamp } of pendingFiles.slice(0, budget)) {
    ...
    try {
      ...
      //   实测最大 14.32 MB → 58,566 事件 → 堆峰值 ~141 MB。
      //   ★ 注意 streamSessionEvents 现在是 **async 且会周期性让出事件循环**：
      //   它省的是内存，而"不再连续占住主线程"靠的是内部的 yield。
      //   漏掉这个 await 会拿到一个 Promise，摘要会静默变成空的。
      const d = await digestSessionFile(s.file, s, streamSessionEvents)
      digests.push(d)
