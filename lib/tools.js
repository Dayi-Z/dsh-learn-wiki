...
import { loadPages, savePage, commitReadiness, CATEGORIES } from './wiki.js'
import { buildCorpus, scoreQuery, triage, recallable } from './recall.js'
import { readGaps, writeGaps } from './acquire.js'
...
export function registerTools(ctx, { getCfg }) {
  const disposers = []
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      const hits = scoreQuery(corpus, args.query)
      ...
      return {
        ...
        note: t.bucket === 'miss'
          ? '未命中：没有已固化知识。可考虑 wiki_learn 沉淀，或让后续轮次的自动补料处理。'
          : undefined,
      }
    },
  })))
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      const slug = String(args.id || args.title || '').toLowerCase()
        .replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 60)
      const id = slug || ('note-' + Date.now().toString(36))
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      return {
        ...
        gaps: gaps.slice(0, 50).map(g => ({ id: g.id, query: g.query, status: g.status, seen: g.seen, attempts: g.attempts, score: g.score })),
