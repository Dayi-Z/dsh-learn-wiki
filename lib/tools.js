...
import { digestSessionFile, renderDigest, sessionTranscriptFile } from './session-digest.js'
  ...
  disposers.push(reg(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (wantSession) {
        ...
        //   而大会话一次读进来实测要 ~370 MB 堆 + 2.5 秒同步阻塞，
        //   这段代码跑在宿主主线程上。现在只留最终那段文本（上限 6000 字符）。
        transcript = sessionTranscriptFile(hit.file, streamSessionEvents, {})
  ...
  disposers.push(reg(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (action === 'show') {
        ...
        // 两条路都走流式：摘要与取材各扫一遍帧，但都**不把会话留在内存里**。
        // 扫两遍比留一份 370 MB 的副本便宜得多。
        const d = digestSessionFile(hit.file, hit, streamSessionEvents)
        const tr = sessionTranscriptFile(hit.file, streamSessionEvents, { maxChars: 3000, maxTurns: 4 })
        return {
