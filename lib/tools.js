...
import { digestSessionFile, renderDigest, sessionTranscriptFile, digestAndTranscriptFile } from './session-digest.js'
  ...
  disposers.push(reg(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (wantSession) {
        ...
        // ★ 流式取材：解一帧喂一帧。原来是把整个会话读成事件数组再交给 runHarvest，
        //   而大会话一次读进来实测要 ~141 MB 堆，这段代码跑在宿主主线程上。
        //   await 不能漏 —— 漏了拿到的是 Promise，取材会静默变成空字符串。
        transcript = await sessionTranscriptFile(hit.file, streamSessionEvents, {})
  ...
  disposers.push(reg(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (action === 'show') {
        ...
        // ★ 摘要与取材**一次扫完**。这两个原本各扫一遍帧 —— 而在大会话上
        //   扫一遍实测 2.8 秒且解压期间不可中断，两遍就是 **4.9 秒主线程无响应**。
        //   在子进程里这毫无症状，在宿主里这会把进程搞崩（这个坑真踩过两次）。
        //   一次扫完 + 内部 yield，代价直接减半。
        // ★ 阶段标记走**同步**日志。宿主在这条路径上原生崩过三次，
        //   而默认的异步写盘内容随进程一起没了 —— 于是"崩在哪一步"完全无从判断。
        trace('show: 开始扫描 ' + hit.id)
        const { digest: d, transcript: tr } = await digestAndTranscriptFile(
          hit.file, hit, streamSessionEvents, { maxChars: 3000, maxTurns: 4 },
        )
