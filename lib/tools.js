...
import { buildMetrics, renderMetrics } from './metrics.js'
import { runHarvest, stageHarvestItems } from './harvest.js'
...
export function registerTools(ctx, { getCfg, llm, caps, log = () => {}, trace = log }) {
  ...
  disposers.push(reg(defineTool({
    ...
    async execute(args, exec) {
      ...
      // ★ 取材失败必须报成失败，不能混进"正常拒绝"里。
      //   实测（2026-09-17）：这条路径曾经把"读不到会话事件"报成
      //   ok:true + skipped:true + 「会话内容太少」+「这是正常的」，
      //   而真相是取材恒为 0 字符 —— 一条故障伪装成了一个业务结论。
      if (res.failed) {
        return {
          ok: false,
          error: res.reason,
          diag: res.diag,
          hint: '这不是"没什么可留的"，是取材那一步没拿到会话 —— 别用手写一页去绕过它，先修取材。',
        }
      }
      if (res.skipped) {
        return {
          ...
          ...(res.diag ? { diag: res.diag } : {}),
          // ★ 解析失败的原始输出要带给调用方 —— 判断是截断、格式还是空回复
          //   就靠它。旧版在这里把 rawLength/rawPreview 丢掉了，于是每次失败
          //   都只有一句「模型没有返回合法 JSON」，完全不可诊断。
          ...(res.rawLength !== undefined ? { rawLength: res.rawLength, rawPreview: res.rawPreview ?? '', rawTail: res.rawTail ?? '' } : {}),
      ...
      //   结果是同一个知识点在库里躺两份。
      //   id 只是文件名；**标题才是给人看的身份**。
      // 落盘走 stageHarvestItems —— 与界面上「从会话提炼」按钮**同一份实现**。
      // 两边各写一份判重，迟早会漂成"按钮出来的页和工具出来的页不一样"。
  ...
  disposers.push(reg(defineTool({
    name: 'wiki_review',
    description: '查看知识库的两段式流水线状态：staged/ 里等待 commit 的暂存页、gaps/ 里检索未命中的缺口队列，以及（detail=metrics 时）命中率与补料收益漏斗。用于人工审阅后台自动补料的产出。只读。',
    parameters: {
      status: prop('string', false, 'gaps 过滤：pending | done | skipped | abandoned | all（默认 all）。'),
      detail: prop('string', false, 'summary（默认）| metrics —— metrics 额外算注入命中率、补料漏斗（缺口→暂存→固化→确认）与知识库利用率。'),
    },
    output: objectOutput,
    async execute(args) {
      const cfg = await getCfg()
      ...
      // ── detail=metrics：命中率与补料收益 ──
      //
      // 刻意做成 wiki_review 的一个 detail 而不是**新工具**：
      // 每多一个工具，每个请求都要多付一份 schema token。
      // 这个项目的 notes 里就写着"加工具时必须来改一次，提醒你代价在哪"。
      if (String(args.detail ?? '') === 'metrics') {
        let logText = ''
        try {
          const { readFile } = await import('node:fs/promises')
          const { join } = await import('node:path')
          logText = await readFile(join(cfg.wikiRoot, '.learn-wiki.log'), 'utf8')
        } catch { logText = '' }   // 拿不到日志 -> buildMetrics 会给出空窗口，而不是假装有数据
        const m = buildMetrics({ logText, gaps: await readGaps(cfg.wikiRoot), pages, usage, now })
        return {
          ok: true, detail: 'metrics',
          window: m.window,
          injections: m.injections,
          acquisition: { ...m.acquisition, rows: m.acquisition.rows },
          library: m.library,
          caveats: m.caveats,
          rendered: renderMetrics(m),
        }
      }
  ...
  disposers.push(reg(defineTool({
    ...
    async execute(args) {
      ...
      // 报**全部站点**的路由，不只是 distill：这个工具的代价主要来自模型调用，
      // 而"它会问谁"完全由配置决定 —— 一次调用就该能看清。
      let routes = {}
      ...
      return {
        // 配置里"下一次会问谁" vs **实际用了谁**：只有后者能解释"为什么这次蒸馏这么慢/这么差"。
        routes,
        used: { distill: llm.used('distill') },
