...
import { listSessions, readSessionEvents } from './session-store.js'
import { digestSession, renderDigest, sessionTranscript } from './session-digest.js'
...
export function registerTools(ctx, { getCfg, llm, caps, log = () => {} }) {
  ...
  disposers.push(reg(defineTool({
    ...
    async execute(args, exec) {
      ...
      let events = null, sessionRef = ''
      if (wantSession) {
        const all = listSessions({ cwd: process.cwd(), includeSubagents: true })
        const hit = all.find(s => s.id === wantSession) ?? all.find(s => s.id.startsWith(wantSession))
        if (!hit) {
          return { ok: false, error: '没找到会话 ' + wantSession, known: all.slice(0, 10).map(s => s.id) }
        }
        events = readSessionEvents(hit.file)
  ...
  disposers.push(reg(defineTool({
    ...
    async execute(args, exec) {
      ...
      // ★「当前工作目录」要取 **agent 会话头里的 cwd**，不是 process.cwd()。
      //
      //   实测（重启后第一次真实调用）：插件的 process.cwd() 是
      //   D:\Harness\dsh-desktop —— 那是 Electron 应用自己的安装目录，
      //   而会话记录的 cwd 是工作区 D:\Harness。两者永远不相等，于是 cwd 过滤
      //   在生产里**从来不命中**，每次都走回退、每次都多带一句说明。
      //   回退是对的（它救了这一次），但正确做法是直接问对的人：
      //   会话头里的 cwd 就是"这个 agent 的工作区"。
      const agentCwd = (() => {
        try { return String(exec?.agent?.session?.header?.cwd ?? '').trim() } catch { return '' }
      })()
      ...
      if (action === 'show') {
        ...
        const ev = readSessionEvents(hit.file)
        const d = digestSession(ev, hit)
        const tr = sessionTranscript(ev, { maxChars: 3000, maxTurns: 4 })
        return {
          ok: true, action: 'show', id: d.id,
          digest: d,
          rendered: renderDigest(d, { maxWalls: 10 }),
