...
import { loadUsage, usageLabel, reinforcementFactor } from './usage.js'
...
export function registerTools(ctx, { getCfg, llm, caps }) {
  const disposers = []
    ...
    async execute(args) {
      ...
      const shape = h => {
        ...
        return {
          ...
          usage: st ? { hits: st.hits ?? 0, confirmed: st.confirmed ?? 0, suspect: st.suspect ?? 0, label: usageLabel(st) } : { label: 'dead' },
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      const buckets = { confirmed: [], suspect: [], unconfirmed: [], dead: [] }
      for (const p of committed) {
        const st = usage.pages[p.id]
        const label = usageLabel(st)
        buckets[label].push({
          id: p.id,
          hits: st?.hits ?? 0, confirmed: st?.confirmed ?? 0, suspect: st?.suspect ?? 0,
          factor: Number(reinforcementFactor(st).toFixed(3)),
        })
      }
      const usageSummary = {
        说明: '命中≠确认。确认 = 命中后该轮没再挣扎；嫌疑 = 命中后仍然挣扎（疑似有害知识）。',
        counts: {
          confirmed: buckets.confirmed.length,
          suspect: buckets.suspect.length,
          unconfirmed: buckets.unconfirmed.length,
          dead: buckets.dead.length,
        },
        // 只列有证据的，死的和未确认的给个数就够
        suspect: buckets.suspect.sort((a, b) => b.suspect - a.suspect).slice(0, 20),
        confirmed: buckets.confirmed.sort((a, b) => b.confirmed - a.confirmed).slice(0, 20),
        dead: buckets.dead.map(x => x.id),
      }
  ...
  // 内置工具缺的正是这一半。
  if (caps) disposers.push(ctx.tools.register(defineTool({
    name: 'find_tools',
