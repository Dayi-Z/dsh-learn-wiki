...
import { findOverlaps } from './merge.js'
 ...
 */
export async function lintWiki(wikiRoot, { pages, usage, policy = DEFAULT_POLICY, now = Date.now(), exists = existsSync, pluginRoot = null, sessionIds = null, overlapFrom = 0.45 } = {}) {
  let ids = sessionIds
  if (!ids) {
    ids = new Set()
    try {
  ...
  // 主题重叠：与重复用**同一把尺子**，只是区间不同（见 merge.js 的注释）。
  // 它报的是"这两页讲的是同一片地面"，**不是**"它们互相矛盾" ——
  // 后者是语义判断，离线代码做不到，假装能做到只会制造噪音。
  const overlaps = findOverlaps(pages, { from: overlapFrom })

  const findings = brokenLinks.length + sources.dead.length + sources.weak.length + duplicates.length + overlaps.length + stale.length
  return {
    ok: true,
    scanned: pages.length,
    brokenLinks, deadSources: sources.dead, weakSources: sources.weak, duplicates, overlaps, stale,
...
export function renderLint(r) {
  ...
  sec('疑似重复', r.duplicates, x => x.a + ' ≈ ' + x.b + '（' + x.score + '，建议合并）')
  sec('主题重叠（可能冲突，需人读）', r.overlaps || [], x => x.a + ' ≈ ' + x.b + '（' + x.score + '）共同词 ' + JSON.stringify(x.sharedTerms))
