...
/** 摘要一个会话（**流式**路径：解一帧喂一帧，不把整个会话读进内存）。 */
export function digestSessionFile(file, header, streamFn) {
  const acc = createDigestAccumulator(header ?? {})
  streamFn(file, (ev) => acc.push(ev))
  return acc.finish()
}
...
/** 取材（**流式**路径：解一帧喂一帧，不把整个会话读进内存）。 */
export function sessionTranscriptFile(file, streamFn, opts = {}) {
  const c = createTranscriptCollector(opts)
  const n = streamFn(file, (ev) => c.push(ev))
