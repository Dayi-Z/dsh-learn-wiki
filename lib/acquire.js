...
export async function runAcquisition({ ctx, llm, repoRoot, cfg, log = () => {}, onStaged = null }) {
  ...
  // ⚠️ 写回时必须重新读一遍再合并，不能拿旧快照整体覆盖。
  //
  // 实测踩到：appendGap 与这里是两条独立的"读-改-写"，后者会把前者
  // 刚写进去的 gap **整个覆盖丢失**。冷却设为 0 时立刻复现 ——
  // 挣扎登记的 gap 被同一时刻启动的补料抹掉了。
  // 保留期间新增的条目，只把「我处理过的」那些字段并回去。
  const latest = await readGaps(repoRoot)
  ...
  await writeGaps(repoRoot, [...byId.values()])
  return summary
}
