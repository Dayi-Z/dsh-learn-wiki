...
async function fetchEvidence(ctx, sources, cfg, log) {
  if (!cfg.fetchSources || typeof ctx?.web?.fetch !== 'function') return []
  const out = []
  for (const s of sources.slice(0, cfg.fetchTopN)) {
    try {
      const r = await ctx.web.fetch({ url: s.url })
      if (r && (r.statusCode === undefined || r.statusCode < 400)) {
        ...
        if (String(text).trim().length > 200) {
          out.push({ url: s.url, title: s.title, text: String(text).slice(0, cfg.fetchMaxChars) })
        }
      }
    } catch (e) {
      log('fetch failed (non-fatal): ' + s.url + ' — ' + (e?.message ?? e))
    }
  }
  return out
}
