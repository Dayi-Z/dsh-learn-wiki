...
function slugify(s) {
  const v = String(s ?? '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 60)
  return v
}
...
export async function acquireOne({ ctx, llm, repoRoot, gap, cfg, log = () => {} }) {
  ...
  const id = slugify(distilled.id) || slugify(distilled.title) || ('gap-' + gap.id)
...
export async function runAcquisition({ ctx, llm, repoRoot, cfg, log = () => {} }) {
  const gaps = await readGaps(repoRoot)
  const pending = gaps.filter(g => g.status === 'pending').slice(0, cfg.maxAcquisitionsPerRun)
