...
export function symptomQuery(signals, context = '') {
  ...
  for (const s of signals) {
    if (s.type === 'recurring-error') {
      ...
      // 只有在**确实有失败证据**时才说"仍不成功"。
      // 原版把这四个字写死了：一个只是"改了很多次"的信号，去搜索引擎那里
      // 声称自己"不成功" —— 查询在撒谎，搜回来的自然也是别的东西。
      const n = Number(s.failCount) || 0
      parts.push('反复修改 ' + baseName(s.file) + (n > 0 ? ' 仍不成功 常见原因' : ' 的实现方式 最佳实践'))
    } else if (s.type === 'repeat-failure') {
      // ★ 优先用**错误指纹**，不要用 detail。
      //   detail 里带着"连续 N 次失败"的计数，而 N 会一路涨 ——
      //   查询每涨一次就变一次，同一个错误在 gap 队列里躺成好几条。
      //   错误指纹是稳定的，而且正是"网上真有人写过"的那部分。
      const sig = String(s.errorSig || '').slice(0, 120)
      parts.push(sig
        ? sig + ' 常见故障原因 解决办法'
        : String(s.tool || s.identity || '').slice(0, 60) + ' 连续失败 常见故障原因 解决办法')
