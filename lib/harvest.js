...
import { CATEGORIES } from './wiki.js'
 ...
 */
export async function runHarvest({ agent, events, sessionRef, llm, focus = '', maxItems = 3, log = () => {} } = {}) {
  // 两条取材路径，同一套取舍规则：
  //   agent  —— 当前会话（agent.session.events 就在手上）
  //   events —— 历史会话（从 session.jsonl.zstd 解出来的事件）
  const ex = Array.isArray(events)
  ...
  if (!j) return { items: [], skipped: true, reason: '模型没有返回合法 JSON', considered: 0, transcriptChars: ex.text.length }
  if (j.skip === true) {
