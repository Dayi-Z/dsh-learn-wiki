 ...
 */
/**
 * 取当前会话的事件 —— **只走宿主真正暴露的接口**。
 *
 * ── 这个函数修的是一个"静默失效"（2026-09-17 查实）──
 *
 * 原来这里读的是 `agent.session.events`：一个**假定存在**的属性。
 * 而 DSH 0.9.0 的 Session 类里根本没有这个成员，它声明的是：
 *
 *   snapshotEvents(fromSeq?, toSeqExclusive?): readonly SessionEvent[]
 *   ownEvents(): readonly SessionEvent[]
 *   eventAt(seq): SessionEvent | undefined
 *
 * （已核对宿主里的 `export class Session` 声明，并用**真的 Session 类**
 *   灌真事件复现过：`'events' in session` 为 false、s.events 为 undefined。）
 *
 * 后果：`Array.isArray(undefined)` 为假 → events 恒为空数组 → 取材恒为
 * **0 字符**。而调用方看到的却是一句
 * 「会话内容太少，不值得提炼（至少 200 字符）」——
 * 一条**看起来完全正常的业务结论**。
 * 实测：真宿主里一次调用返回 transcriptChars=0；同一段对话按会话 id
 * 从磁盘提炼却有 3591 字符。
 *
 * 两种解释（"这段对话没什么可留的" vs "我根本没读到这段对话"）在行为上
 * 完全相反，旧代码却把它们合并成了同一句话。这正是本项目最忌讳的形态，
 * 所以这里不但要取对，还要**把"取不到"这件事单独说出来**（见 unreadable）。
 *
 * @returns { events, via, error } events 为 null 表示**读不到**，
 *          与"读到了但没有内容"是两件事。
 */
export function readSessionEvents(agent) {
  const s = agent?.session
  if (!s) return { events: null, via: '', error: '执行上下文里没有 agent.session（拿不到当前会话对象）' }
  // ★ 顺序有意为之：snapshotEvents() 给的是**整条日志**，ownEvents() 只给
  //   fork 之后属于本会话的那一段 —— 被 fork 出来的会话用它取会**丢掉继承的
  //   历史**，所以它只能是退路，不能是首选。老宿主的 events 属性排最后。
  for (const name of ['snapshotEvents', 'ownEvents']) {
    if (typeof s[name] !== 'function') continue
    try {
      const ev = s[name]()
      if (Array.isArray(ev)) return { events: ev, via: 'session.' + name + '()' }
      return { events: null, via: 'session.' + name + '()', error: 'session.' + name + '() 返回的不是数组（是 ' + typeof ev + '）' }
    } catch (e) {
      return { events: null, via: 'session.' + name + '()', error: 'session.' + name + '() 抛错：' + String(e?.message ?? e) }
    }
  }
  // 旧宿主（events 还是数组属性的那几版）仍然认，**只是排在真接口后面**。
  if (Array.isArray(s.events)) return { events: s.events, via: 'session.events（旧属性）' }
  // 走到这里 = 宿主换了接口名。把**试过哪些**写进错误里，下次一眼就对得上。
  return { events: null, via: '', error: 'agent.session 上找不到取事件的接口（试过 snapshotEvents() / ownEvents() / events 属性）' }
}

export function extractSessionText(agent, opts = {}) {
  const { events, via, error } = readSessionEvents(agent)
  // 会话锚点优先用 session.id：它是 Session 自己的身份。agent.id 兜底。
  const id = agent?.session?.id ?? agent?.id
  const sessionRef = id ? 'session://' + id : ''
  if (!events) {
    return { text: '', turns: 0, chars: 0, truncated: false, sessionRef, via, unreadable: error, eventsScanned: 0 }
  }
  return { ...sessionTranscript(events, opts), sessionRef, via, eventsScanned: events.length }
}

/** 从**历史会话**的事件里取材（与实时路径共用同一套取舍规则）。 */
export function extractEventsText(events, sessionRef, opts = {}) {
  const list = Array.isArray(events) ? events : []
  return { ...sessionTranscript(list, opts), sessionRef: sessionRef ?? '', via: '磁盘事件', eventsScanned: list.length }
}
...
export async function runHarvest({ agent, events, transcript, sessionRef, llm, focus = '', maxItems = 3, maxTokens = 3000, log = () => {} } = {}) {
  ...
  //                   而这段跑在宿主主线程上）
  const ex = transcript
    ? { text: transcript.text, turns: transcript.turns ?? 0, chars: transcript.chars ?? 0, sessionRef: sessionRef ?? '', via: '调用方给的 transcript' }
    : Array.isArray(events)
      ? extractEventsText(events, sessionRef, {})
      : extractSessionText(agent, {})

  // ★ 「读不到」≠「没内容」—— 这两件事在行为上完全相反：
  //   前者是**故障**（取材这一步坏了，得修），后者是**结论**（这段对话确实
  //   没什么值得长期保留的）。旧代码把它们合并成同一句「会话内容太少」，
  //   于是取材取到 0 字符时，工具照样回 ok:true + 「这是正常的」。
  //   所以这里必须分开报，而且读不到要报成 failed。
  if (ex.unreadable) {
    log('harvest: 读不到会话事件 —— ' + ex.unreadable + '（via=' + (ex.via || '无可用接口') + '）')
    return {
      items: [], skipped: true, failed: true,
      reason: '读不到当前会话的事件，这次提炼**根本没有发生**：' + ex.unreadable,
      diag: { via: ex.via || '(没有可用的取事件接口)', sessionRef: ex.sessionRef ?? '', eventsScanned: null },
      considered: 0, transcriptChars: 0,
    }
  }
  if (!ex.text || ex.text.length < 200) {
    return {
      items: [], skipped: true,
      reason: '会话内容太少，不值得提炼（至少 200 字符）',
      considered: 0, transcriptChars: ex.text.length,
      // ★ 把取材事实一并带出来：这样「扫了 214 个事件、确实只提出 12 个字符」
      //   与「一个事件都没扫到」在**报告里**也分得开，不必再靠读代码去猜。
      diag: { via: ex.via ?? '', eventsScanned: ex.eventsScanned ?? null },
    }
  }
  ...
  // ★ 带契约校验解析：模型思考过程里可能出现**完整可解析的示例对象**
  //   （比如 {"skip": true}），而真正的答案被截断时，"闭合最靠后"规则
  //   会选中那个示例。示例不符合本函数要求的形状 —— skip 必须是布尔，
  //   且 skip:true 带 reason、skip:false 带 items 数组 —— 所以用 ok 过滤。
  //   实测（2026-09-17）：len=10215 的输出开头是 "Here's a thinking process:"，
  //   思考里就带着输出形状的示例。
  const okShape = (v) => typeof v?.skip === 'boolean'
    && (v.skip === true ? typeof v.reason === 'string' : Array.isArray(v.items))
  const j = extractJson(raw, okShape)
  if (!j) {
    ...
    // ★ 开头+结尾一起记。判断是**截断**还是**格式**就靠这两头：
    //   结尾缺配平（没有收尾的 }）→ 大概率截断 → 调 maxTokens；
    //   结尾是完整 JSON 但开头有思考过程 → 格式污染 → 已由 extractJson 兜底。
    //   只记开头的话，这两种情况长得一模一样。
    log('harvest: 解析失败 len=' + text.length
      + ' 开头=' + JSON.stringify(text.slice(0, 160))
      + ' 结尾=' + JSON.stringify(text.slice(-160)))
    return {
  ...
  // 取材路径（via）每次都记：接口一旦漂移，日志里这一栏就是第一手证据。
  log('harvest: ' + items.length + ' 条候选（会话 ' + ex.chars + ' 字符 / ' + ex.turns + ' 轮，取材 ' + (ex.via || '?') + '）')
